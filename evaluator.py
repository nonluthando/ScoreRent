...
def test_bursary_student_rent_equal_support_has_no_affordability_penalty():
    result, bands = evaluate(
        renter_type="student",
        monthly_income=8000,
        renter_docs=["proof_of_registration", "bursary_letter"],
    ...
    assert result.score >= 100 or result.score >= 90
    assert any("covers rent" in r.lower() or "affordability penalty skipped" in r.lower() for r in result.reasons)
...
def test_new_professional_rewards_employment_contract():
